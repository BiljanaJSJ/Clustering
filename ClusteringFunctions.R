...
# mat=ExtractFeatures(DataSet,Features=c('General Risk'))
# 
# fit <- Mclust(mat)
...
# 
# 
# clplot=clusplot(mat, fit$classification, color=TRUE, shade=TRUE, 
#          labels=2, lines=0)
# 
# #randomly select users to show on a plot
# nUsers=15
# 
# png(paste('clusterPlot',nUsers,'.png',sep=''))
# 
# plotSelectedUsers=sample(mat[,1],size=nUsers)
# clusplot(mat[plotSelectedUsers,], fit$classification[plotSelectedUsers], color=TRUE, shade=TRUE, 
#                 labels=2, lines=0,main=paste('Plot of',nUsers, 'randomly selected users'))
# 
# dev.off()
# 
# png('PlotClust.png')
# plotcluster(mat, fit$classification)
# dev.off()
# 
# png('PlotClustUser.png')
# plotcluster(mat[,1], fit$classification, xlab='General Risk',ylab='clusters')
# dev.off()


# cluster_Kmeans=kmeans(DataSet[[1]][,4], centers=5, iter.max = 10, nstart = 1,
#                       algorithm = c("Hartigan-Wong", "Lloyd", "Forgy",
#                                     "MacQueen"), trace=FALSE)
# 
# plot(DataSet[[1]][,5],col=cluster_Kmeans$centers)
# points(cluster_Kmeans[[1]], col =cluster_Kmeans$centers , pch = 8, cex = 2)
# 
# 
# cbind(DataSet[[1]][,5],cluster_Kmeans$centers)
# 
# 
# fit = kmeans(DataSet[[1]][,5], 5) # 5 cluster solution
# # get cluster means 
# aggregate(DataSet[[1]][,5],by=list(fit$cluster),FUN=mean)
# # append cluster assignment
# DS = data.frame(DataSet[[1]][,5], fit$cluster)
# 
# 
# clusplot(DataSet[[1]][,c(1,5)], fit$cluster, color=TRUE, shade=TRUE, 
#          labels=2, lines=0)
# 
# 
# plotcluster(DataSet[[1]][,5], fit$cluster, xlab='General Risk',ylab='clusters')
# 
# 
# 
# 
# fit1 <- kmeans(DataSet[[1]][,5], 3) # 5 cluster solution
# fit2 <- kmeans(DataSet[[1]][,5], 4) # 5 cluster solution
# cluster.stats(dist(DataSet[[1]][,5]), fit1$cluster, fit2$cluster)


# fit = kmeans(mat, 5) # 5 cluster solution
# # get cluster means 
# aggregate(mat,by=list(fit$cluster),FUN=mean)
# # append cluster assignment
# DS = data.frame(mat, fit$cluster)
# 
# 
# clusplot(mat, fit$cluster, color=TRUE, shade=TRUE, 
#          labels=2, lines=0)
# 
# plotcluster(mat, fit$cluster, xlab='General Risk',ylab='clusters')
# 
# fit=hclust(mat1)
# 
# 
